...
sel_sentiment = st.selectbox("Filter by Sentiment", ["All", "positive", "negative", "neutral"])
...
if sel_stage != "All": filtered = [i for i in filtered if i["journey_stage"] == sel_stage]
if sel_sentiment != "All": filtered = [i for i in filtered if i["sentiment"] == sel_sentiment]
...
if total_pages > 1:
    ...
    display_items = filtered[start:end]
    st.markdown(f"<p style='color:#666;font-size:0.8rem;'>Page {page} of {total_pages}</p>", unsafe_allow_html=True)
else:
    display_items = filtered[:page_size]
