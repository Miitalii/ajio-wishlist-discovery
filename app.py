...
@st.cache_data
def load_data():
    with open("ajio_data.json", "r") as f2:
        return json.load(f2)
...
<style>
    ...
    .source-grid { background: #1a1d29; border-radius: 12px; padding: 16px; text-align: center; }
    .source-number { font-size: 1.8rem; font-weight: 700; color: #ffffff; }
    .source-name { font-size: 0.8rem; color: #8888a0; margin-top: 4px; }
</style>
"""
...
st.sidebar.markdown("---")
st.sidebar.markdown("""**Goal:** Increase % of users who purchase at least 1 wishlist item within 30 days.

**Approach:** AI-powered analysis of public user feedback mapped to the wishlist-to-purchase journey.""")

view = st.sidebar.radio("Select View", ["📊 Executive Dashboard", "🔍 Thematic Analysis", "🏆 Opportunity Ranker", "🧪 Live Analyzer", "📖 Methodology"])
...
if view == "📊 Executive Dashboard":
    ...
    st.info("""**How to read this:** PRE_PURCHASE issues are direct conversion blockers — users are actively considering a wishlist item but something stops them. POST_DELIVERY issues are trust eroders — they make users hesitant to buy from their wishlist in the future.""")
    ...
    user_input = st.text_area("Paste user feedback here:", height=120, placeholder="Example: I loved the kurta design but I have no idea if it will fit me. The size chart is confusing and there are no reviews with photos.")
...
st.markdown("---")
st.markdown("""<div style="text-align: center; color: #666; font-size: 0.8rem;">AJIO Wishlist Discovery Engine | Built for PM Fellowship Graduation Project | 2026</div>""", unsafe_allow_html=True)
