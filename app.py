...
def get_theme_color(theme):
    colors = {
        "FIT_UNCERTAINTY": "#e74c3c",
        "QUALITY_DOUBT": "#e67e22",
        "TRUST_DEFICIT": "#9b59b6",
        "INFO_GAP": "#3498db",
        "RETURN_NIGHTMARE": "#e74c3c",
        "APP_FRICTION": "#95a5a6",
        "PRICE_SATISFACTION": "#27ae60",
        "FORGOTTEN_WISHLIST": "#8e44ad",
        "EXTERNAL_COMPARISON": "#16a085"
    }
...
if view == "📊 Executive Dashboard":
    ...
    with col1:
        st.metric("Total Data Points", len(insights))
    with col2:
        sources = len(set(i["source"] for i in insights))
        st.metric("Sources Analyzed", sources)
    with col3:
        themes = len(set(i["theme"] for i in insights))
        st.metric("Themes Identified", themes)
    with col4:
        top_theme = ranking[0]["theme_label"]
        st.metric("Top Opportunity", top_theme)
    ...
    journey_df = pd.DataFrame({
        "Journey Stage": list(journey_counts.keys()),
        "Issues": list(journey_counts.values())
    })
    
    st.bar_chart(journey_df.set_index("Journey Stage"))
...
# VIEW 2: Thematic Analysis
    ...
    if selected_theme != "All":
        filtered = [i for i in filtered if i["theme_label"] == selected_theme]
    if selected_stage != "All":
        filtered = [i for i in filtered if i["journey_label"] == selected_stage]
    ...
    for item in filtered:
        ...
        <div class="insight-card {opp_class}">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                ...
                <div style="color: #666; font-size: 0.8rem;">{item["source"]} | Opportunity Score: <b>{item.get("opportunity_score", "N/A")}</b></div>
            </div>
            <div style="font-style: italic; color: #333; margin: 8px 0;">"{item["raw_text"]}"</div>
            <div style="font-size: 0.85rem; color: #666;">
                <b>Impact:</b> {item["impact_on_conversion"]} | 
                <b>Frequency:</b> {item["frequency_score"]}/100 | 
        ...
        with st.container():
            ...
            <div class="insight-card {opp_class}">
                <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                    <div style="flex: 1;">
                        <div style="font-size: 1.3rem; font-weight: 700; color: #1a1a2e; margin-bottom: 4px;">
                            #{i+1} {opp["theme_label"]}
                        </div>
                    ...
                    <div style="background: {'#e74c3c' if score >= 80 else '#f39c12' if score >= 60 else '#27ae60'}; 
                                color: white; padding: 8px 16px; border-radius: 20px; font-weight: 700; font-size: 1.2rem; margin-left: 16px;">
                        {score}
                    </div>
                </div>
            </div>
...
# VIEW 4: Live Analyzer
    ...
    if st.button("Analyze Feedback", type="primary"):
        if user_input.strip():
            ...
            themes_detected = []
            if any(word in text_lower for word in ["size", "fit", "fitting", "tight", "loose", "chart", "measurement", "sizing"]):
                themes_detected.append(("FIT_UNCERTAINTY", "Fit & Sizing Confidence", 92))
            if any(word in text_lower for word in ["quality", "cheap", "fabric", "material", "stitching", "color different", "see-through"]):
                themes_detected.append(("QUALITY_DOUBT", "Product Quality Mismatch", 85))
            if any(word in text_lower for word in ["review", "rating", "photo", "picture", "no one bought", "social proof"]):
                themes_detected.append(("INFO_GAP", "Missing Social Proof & Reviews", 88))
            if any(word in text_lower for word in ["return", "refund", "pickup", "exchange", "customer care", "customer service"]):
                themes_detected.append(("RETURN_NIGHTMARE", "Return & Refund Friction", 70))
            if any(word in text_lower for word in ["trust", "scam", "fraud", "never again", "elsewhere", "deteriorated"]):
                themes_detected.append(("TRUST_DEFICIT", "Platform Trust Erosion", 78))
            if any(word in text_lower for word in ["price", "expensive", "discount", "sale", "costly", "cheap price"]):
                themes_detected.append(("PRICE_WAITING", "Price Sensitivity", 55))
            if any(word in text_lower for word in ["app", "ui", "cluttered", "slow", "ads", "notification"]):
                themes_detected.append(("APP_FRICTION", "App Experience Friction", 45))
            if any(word in text_lower for word in ["forget", "forgot", "remember", "reminder", "lost interest"]):
                themes_detected.append(("FORGOTTEN_WISHLIST", "Wishlist Discovery & Reminders", 55))
            if any(word in text_lower for word in ["myntra", "amazon", "compare", "comparison", "check other"]):
            ...
            st.info("""
            **Note:** This demo uses rule-based classification. In the full version, this would call an LLM (GPT-4/Claude) 
            for nuanced semantic analysis. The framework and scoring logic remain the same.
            """)
        else:
